...
def rag_chain():
    ...
    prompt = PromptTemplate.from_template(
        """
        You are a friendly assistant made by IWAYPlus to help users find information about the RNI building. 
        General information about the building:
        The context includes similar questions and answers to what the user might ask.
        Use general information first to answer, if it's not possible, use the context.
        Question: {input} 
        Similar Question Answers: 
            {
            General Building Info:
                ...
                Each floor has rooms like 1A-1A (1st floor block A), 2B-2C (Second floor block B) etc.
                The building has eateries like Breadbox and a cafeteria.
...
def chat_endpoint():
    ...
    response = chat(user_message)  # Now the chat function is called here too
